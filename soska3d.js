...
export function createSoska(canvas){
  ...
  const black=new THREE.MeshPhysicalMaterial({color:0x0b0b0c,metalness:.1,roughness:.28,clearcoat:.6,clearcoatRoughness:.25});
  ...
  { const c=document.createElement('canvas');c.width=1024;c.height=360;const g=c.getContext('2d');
    const gr=g.createLinearGradient(0,0,0,360);gr.addColorStop(0,'#e9cf8c');gr.addColorStop(.5,'#c9a45a');gr.addColorStop(1,'#a8823e');
    g.fillStyle=gr;g.fillRect(0,0,1024,360);g.strokeStyle='rgba(80,55,15,.55)';g.lineWidth=6;g.strokeRect(14,14,996,332);
    g.fillStyle='#3d2a0c';g.textAlign='center';g.font='900 86px Helvetica, Arial';g.fillText('RADIO AWARDS',512,150);
    g.font='700 44px Helvetica, Arial';g.fillText('PLZEŇSKÝ KRAJ  ·  2026',512,240);
    const t=new THREE.CanvasTexture(c);t.colorSpace=THREE.SRGBColorSpace;
    add(new THREE.BoxGeometry(2.5,.86,.03),new THREE.MeshPhysicalMaterial({map:t,metalness:.85,roughness:.32})).position.set(0,BASE_H/2,1.66); }
